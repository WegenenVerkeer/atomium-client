 ...
 * feed) is a separate opt-in capability: implement {@link FeedPusher} next to the handler variant. The
 * handler is <em>pure domain</em> (identity + callbacks): infrastructure config (HTTP client, decoder,
 * executor, …) lives in the {@link Feed}, not here.
 *
 ...
 * (start, page boundary, end of feed, interruption), implement a {@link FeedEventListener} — there those events
